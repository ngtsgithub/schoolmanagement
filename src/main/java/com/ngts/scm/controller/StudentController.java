...
public class StudentController {
	...
	public ResponseEntity<?> save(@Valid @RequestBody StudentVO vO) {
		...
		// MessageResponse(studentService.save(vO).toString()));
		...
		try {
			...
			return ResponseEntity.badRequest()
					.body(new MessageResponse("Please Enter EmailStudent Id created in the System !"));
		}
	...
	@PutMapping("/{id}")
	public void update(@Valid @NotNull @PathVariable("id") Integer id, @Valid @RequestBody StudentUpdateVO vO) {
	 ...
	 * 
	 * @ApiOperation("Retrieve by query ") public Page<StudentDTO> query(@Valid
	 * StudentQueryVO vO) { return studentService.query(vO); }
	 */
	@GetMapping("/all")
